...
<div className="w-11/12 max-w-[500px] grid grid-cols-2 gap-x-[6px] gap-y-[6px] self-center text-black">
            {prompts?.map((item, index) => (
                <div key={index} className= "flex transition-border-color ease duration-300 flex-row rounded-lg py-2 px-2 text-[14px] border-[1.5px] leading-snug h-[80px] hover:border-[#101625]" onMouseEnter={() => setHoveredPrompt(item)} onMouseLeave={() => setHoveredPrompt()}>                      
